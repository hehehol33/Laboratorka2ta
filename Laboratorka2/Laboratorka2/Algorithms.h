...
const int V = 11; // Да, дубликат, но так уж получилось, что структуре тоже над размерность знать( 
...
void PathIntoString(ShortestPathResult& result, const std::string Places[]); // Эту исполняем после отработки алгоритма, ты ей кормишь структуру и местечка, 
// а он вам в структуру назад пишет все эт добро по ссылке, ток бери потом и по индексам бегай
// 
// Раньше была еще и выводом в консося, так чт если есть вопросы по выводу, можете разкоментить две строчки и дописать аргументы в фунцию, и чекать
